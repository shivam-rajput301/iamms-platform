...
function formatAsset(a) {
  ...
  return {
    ...
    department_id: a.department_id
      ? a.department_id._id?.toString() || a.department_id.toString()
      : null,
    plant: a.plant,
    location: a.location,
...
function formatRequest(r) {
  return {
    ...
    status: r.status,
    requested_by:
      r.requested_by?._id?.toString() || r.requested_by?.toString() || "",
    assigned_engineer:
      r.assigned_engineer?._id?.toString() ||
      r.assigned_engineer?.toString() ||
      null,
    assigned_by:
      r.assigned_by?._id?.toString() || r.assigned_by?.toString() || null,
    images: r.images || [],
    ...
    requester:
      typeof r.requested_by === "object"
        ? formatProfile(r.requested_by)
        : undefined,
    engineer:
      typeof r.assigned_engineer === "object"
        ? formatProfile(r.assigned_engineer)
        : null,
    assigner:
      typeof r.assigned_by === "object" ? formatProfile(r.assigned_by) : null,
  };
}
...
function formatLog(l) {
  return {
    ...
    author:
      typeof l.author_id === "object" ? formatProfile(l.author_id) : undefined,
  };
}
...
router.post("/", async (req, res) => {
  try {
    ...
    const requestCode =
      req.body.request_code || `REQ-2026-${String(count + 1).padStart(3, "0")}`;
