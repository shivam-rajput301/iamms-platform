...
import { adminApi, organizationApi, getApiToken, type OrganizationConfig } from '@/lib/api';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: '/requests',     label: 'Maintenance',   icon: Wrench,          permission: 'requests:view' },
  ...
  { to: '/profile',      label: 'Profile',       icon: User,            permission: 'dashboard:view' },
];
...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const items = NAV_ITEMS
    .filter((item) => canFn(item.permission))
    .map((item) => {
  ...
  useEffect(() => {
    ...
    organizationApi.getConfig()
      .then((res) => { if (!cancelled && res) setOrgConfig(res); })
      .catch(() => {});

    if (isSuperAdmin && getApiToken()) {
      adminApi.getPendingCount()
        .then((res) => { if (!cancelled) setPendingCount(res.count); })
        .catch(() => {});
  ...
  return (
    <>
      {open && <div className="fixed inset-0 z-30 bg-steel-950/40 dark:bg-steel-950/60 backdrop-blur-sm lg:hidden" onClick={onClose} />}
      <aside
        className={cn(
      ...
      >
        ...
        <div className="border-b border-steel-200 dark:border-steel-800 p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="truncate text-base font-extrabold tracking-tight text-steel-900 dark:text-white">{APPLICATION_NAME}</h1>
              <p className="truncate text-[10px] font-medium text-steel-500 dark:text-steel-400">{APPLICATION_FULL_NAME}</p>
            </div>
            <button onClick={onClose} className="ml-auto rounded-lg p-1 text-steel-400 hover:bg-steel-100 dark:hover:bg-steel-800 lg:hidden">
              <X className="h-4 w-4" />
            </button>
          </div>

          {/* Subtitle: Organization Config */}
          <div className="mt-3 rounded-lg border border-steel-200 bg-steel-50 p-2.5 dark:border-steel-800/80 dark:bg-steel-950/60">
            ...
            <p className="mt-1 truncate text-xs font-semibold text-steel-900 dark:text-steel-200">
              {orgConfig.company_name || 'Not Configured'}
            </p>
            <p className="truncate text-[11px] font-medium text-steel-600 dark:text-steel-400">
              {orgConfig.plant_name || 'Not Configured'}
            </p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-thin px-3 py-4">
          ...
          {isSuperAdmin && (
            <div className="mt-5 pt-4 border-t border-steel-200 dark:border-steel-800/80">
              ...
              <NavLink to="/admin/pending-registrations" onClick={onClose} className={navLinkClass}>
        ...
        <div className="border-t border-steel-200 dark:border-steel-800 p-3">
          <div className="flex items-center gap-3 rounded-lg bg-steel-50 border border-steel-200 p-2.5 dark:bg-steel-950/60 dark:border-steel-800/60">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-semibold text-steel-900 dark:text-steel-200">{profile?.full_name}</p>
              <p className="truncate text-[10px] text-steel-500 dark:text-steel-400">{profile?.designation ?? 'Authorized Personnel'}</p>
            </div>
          </div>
        </div>
      </aside>
